...
def cmd_dividends(portfolio_path: str) -> None:
    portfolio = _load(portfolio_path)
    projection = dividends.project_dividends(portfolio, SampleMarketAdapter())
    click.echo(dividends.render(projection))


@main.command("options")
@click.option("--portfolio", "portfolio_path", required=True, type=click.Path(exists=True))
def cmd_options(portfolio_path: str) -> None:
    portfolio = _load(portfolio_path)
    ideas = options.scan_covered_calls(portfolio, SampleMarketAdapter())
...
def cmd_run(portfolio_path: str, out: str, drafts_dir: str) -> None:
    portfolio = _load(portfolio_path)
    digest = build_digest(portfolio, drafts_dir=Path(drafts_dir))
